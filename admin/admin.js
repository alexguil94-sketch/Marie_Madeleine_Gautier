(function(){
  ...
  function setView(name){
    ...
    navBtns.forEach(b=> b.classList.toggle('is-active', b.getAttribute('data-view') === active));
    views.forEach(v=>{
  ...
  function bindDropzone({ inputId, dropId, previewId, metaId, maxFiles=10, multiple=true }){
    ...
    input.addEventListener('change', ()=>{
      ...
      if(files.length > maxFiles){
        setInputFiles(input, files.slice(0, maxFiles));
      }
      render();
    });
  ...
  async function refreshPublications(){
    ...
    data.forEach(p=>{
      ...
      item.innerHTML = `
        ...
        <div class="admin-actions">
          <button class="pill" type="button" data-toggle-pub="${p.id}" data-next="${p.is_published ? '0' : '1'}">${p.is_published ? 'Dépublier' : 'Publier'}</button>
  ...
  async function init(){
    ...
    if(mediaTypeSel){
      const update = ()=>{
        ...
        if(v === 'youtube'){
          mediaImageWrap.style.display = 'none';
          mediaYoutubeWrap.style.display = '';
        }else{
          mediaImageWrap.style.display = '';
          mediaYoutubeWrap.style.display = 'none';
        }
      };
  ...
  workForm?.addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      const folder = `works/${new Date().toISOString().slice(0,10)}`;
      ...
      setMsg(workMsg, "✅ Œuvre publiée.");
      workForm.reset();
      // reset previews
      const input2 = document.getElementById('workImages');
      if(input2) input2.value = '';
      // trigger re-render previews
  ...
  pubForm?.addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      // Use a stable folder per publication
      const pubId = (window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`);
